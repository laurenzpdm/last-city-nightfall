...
func headline() -> String:
	...
	if frozen > 0:
		return "%d building%s frozen%s" % [frozen, "" if frozen == 1 else "s",
			", grid %.0f heat/s short" % deficit if deficit > 0.5 else ""]
	if deficit > 0.5:
		var who: String = ""
		if not bottlenecks.is_empty():
			...
			if String(b.get("reason", "")) == "capacity":
				who = " — %s at (%d, %d) is the limit, %.0f/%.0f heat/s" % [
		...
		return "%.0f heat/s short%s" % [deficit, who]
	...
	if not bottlenecks.is_empty():
		...
		if String(b2.get("reason", "")) == "capacity":
			return "%s at (%d, %d) is at its limit — %.0f/%.0f heat/s, nothing lost yet" % [
