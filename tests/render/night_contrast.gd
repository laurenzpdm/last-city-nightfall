...
## ── HOW IT FAILS ─────────────────────────────────────────────────────────────
##
## Restore `LcnEntityRenderer.RIM_DELTA`/`BODY_DELTA` to 0, or `_hide`'s old
## `lerpf(tint * 0.32, 0.055, 0.86)`, and the median delta-L falls to about 0.09
## and FOUND falls to zero of eighteen. That was run, against the reverted files,
## before this suite was believed. See the header of `_draw_agent_edge`.
...
## Deep night, driven through SimClock the way the game drives it.
const HOUR_T: float = 0.00
...
func _stage() -> void:
	...
	if _step == 0:
		_place_wave(model)
		SimClock.tick = int(fposmod(HOUR_T - 0.22, 1.0) * 40.0 * 20.0)
		if _cam == null:
	...
	if _step <= SETTLE_FRAMES:
		# The control plate: the same night, the same camera, nobody in it. It
	...
	if _bare == null:
		...
		if _bare != null:
			_bare.save_png(ProjectSettings.globalize_path("%s/wave_bare.png" % OUT))
	...
	if img == null or _bare == null:
		...
		_finish(126)
		return
	img.save_png(ProjectSettings.globalize_path("%s/wave.png" % OUT))
	_judge(img, _bare)


## A converging arc of hostiles, walking in at the settlement across open ground.
##
...
## in which every creature faces the same way is not the wave the game delivers.
func _place_wave(model: LcnWorldModel) -> void:
	...
	for i: int in WAVE:
		...
		_spots.append({"kind": String(kind), "pos": p})


# ------------------------------------------------------------------ judging --

func _judge(lit: Image, bare: Image) -> void:
	...
	for spot: Dictionary in _spots:
		...
		if float(r["delta"]) >= MIN_DELTA and int(r["blob"]) >= MIN_BLOB_PX \
			...
			found += 1
	print("────────────────────────────────────────────────────────────────")
	print(" night contrast — a wave of %d at deep night, zoom %.2f" % [WAVE, JUDGE_ZOOM])
	print("────────────────────────────────────────────────────────────────")
	print("  %-20s %7s %7s %7s %7s %6s" % ["kind", "ground", "peak", "deltaL", "blob", "own"])
	for r2: Dictionary in rows:
		print("  %-20s %7.4f %7.4f %7.4f %7d %6.2f" % [
			r2["kind"], float(r2["ground"]), float(r2["peak"]), float(r2["delta"]),
			int(r2["blob"]), float(r2["own"])])
	deltas.sort()
	...
	print("  median deltaL %.4f, worst %.4f — the bar is %.2f per creature, %.2f median"
		% [median, worst, MIN_DELTA, MIN_MEDIAN_DELTA])
	print("  %d of %d creatures resolve as a distinct blob of >= %d px they own; "
		% [found, rows.size(), MIN_BLOB_PX]
		+ "%d reach the blob size at all" % blobs)

	var fails: Array[String] = []
	if not _chrome_left.is_empty():
		fails.append("chrome above [P13]'s post layer was still in the photograph: %s"
			% ", ".join(_chrome_left))
	if rows.size() < MIN_FOUND:
		fails.append(("only %d of the wave landed inside the frame — the camera is "
			+ "not looking at what this suite staged, so nothing here was measured")
			% rows.size())
	if median < MIN_MEDIAN_DELTA:
		fails.append(("the wave is not separated from the ground it walks on: median "
			+ "deltaL %.4f against a bar of %.2f. Half the night is under it.")
			% [median, MIN_MEDIAN_DELTA])
	if found < MIN_FOUND:
		fails.append(("%d of %d creatures resolve as a distinct high-contrast blob, "
			+ "and the bar is %d. The agent counter is not the picture.")
			% [found, rows.size(), MIN_FOUND])
	if offscreen > 0:
		print("  note: %d staged creature(s) fell outside the frame and were not graded"
			% offscreen)
	if fails.is_empty():
		print("  the night is on screen")
		print("TESTS PASSED")
		_finish(0)
		return
...
##   peak    the strongest departure from that ground anywhere inside the box.
...
func _grade_one(lit: Image, bare: Image, x0: int, y0: int) -> Dictionary:
	...
	for yy: int in BOX_H:
		for xx: int in BOX_W:
			...
			mine[yy * BOX_W + xx] = 1 if absf(l - b) > 0.012 else 0
